...
namespace Meangpu.Gacha
{
    ...
    public class GachaManager : MonoBehaviour
    {
        ...
        [SerializeField] bool _createPreviewOnStart = true;

        private void Start()
        {
            if (_updateDictOnStart) ActionGacha.OnCurrentDictUpdate(_dictionaryGachaCount);
            if (_createPreviewOnStart) CreatePreviewGameObject();
        }
        ...
        public GameObject GetObjectFromPreview()
        {
            ...
            float percentToGetThis = _dictionaryGachaCount.FirstOrDefault(x => x.Key.name == finalObject.name).Value / (float)_dictionaryGachaCount.Values.Sum();
